            ...
            try:
                ...
                for agent_tool_handle in project.list_agent_tools(as_type="objects"):
                    try:
                        ...
                        print(
                            f"[EXCEPTION] generate rows - llm Usage - agent tool: {e}"
                        )
                    finally:
                        if not next_row["llmId"]:
                            continue
                ...
                for handle in project.list_knowledge_banks(as_type="objects"):
                    try:
                        next_row = {
                            "projectKey": project_key,
                            "dss_object_type": "knowledge bank",
                        }
                        ...
                        print(
                            f"[EXCEPTION] generate rows - llm Usage - knowledge bank: {e}"
                        )
                    finally:
                        if not next_row["llmId"]:
                            continue
                ...
                for handle in project.list_recipes(as_type="objects"):
                    try:
                        next_row = {
                            "projectKey": project_key,
                            "dss_object_type": "recipe",
                        }
                        next_row["dss_object_id"] = handle.id
                        next_row["dss_object_name"] = handle.name
                        data = handle.get_settings().get_recipe_raw_definition()

                        next_row["llmId"] = recursive_search_all(
                            data, "llmId"
                        ) or recursive_search_all(data, "embeddingLLMId")
                    except Exception as e:
                ...
                for handle in project.list_retrieval_augmented_llms(as_type="objects"):
                    try:
                        next_row = {
                            "projectKey": project_key,
                            "dss_object_type": "retrieval augmented LLM",
                        }

                        next_row["dss_object_name"] = (
                            None  # they don't have a name, or they're stored inside the active version info
                        )
                        data = handle.get_settings().get_raw()
